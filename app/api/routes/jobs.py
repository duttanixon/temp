...
async def sync_job_status(db: AsyncSession, job_obj: Job) -> Job:
    """
    ...
    if job_obj.status not in [JobStatus.SUCCEEDED, JobStatus.FAILED, JobStatus.TIMED_OUT, JobStatus.CANCELED, JobStatus.ARCHIVED]:
        ...
        device_obj = await device.get_by_id(db, device_id=job_obj.device_id)
        if device_obj and device_obj.thing_name:
            ...
            if aws_status:
                ...
                if new_status and new_status != job_obj.status:
                    job_obj = await job.update_status(
                        db,
                        job_id=job_obj.job_id,
                        status=new_status,
...
async def create_restart_jobs_task(
        ...
        try:
            for device_id in job_in.device_ids:
                try:
                    ...
                    await check_device_access(current_user, db_device, action="create jobs for")
                    ...
                    db_job = await job.create_with_device_update(
                        db_session, obj_in=job_create, job_id=aws_job_info["job_id"], user_id=current_user.user_id
                    )
                    ...
                    await log_action(
                        db=db_session, user_id=current_user.user_id, action_type=AuditLogActionType.JOB_CREATE,
...
async def create_reboot_jobs_task(
...
async def create_restart_application_job(
    *,
...
async def create_reboot_device_job(
    *,
...
async def get_device_jobs(
    *,
    ...
    # Get jobs
    jobs_list = await job.get_by_device(db, device_id=device_id, skip=skip, limit=limit)
    
    # Sync status for non-terminal jobs
    response = []
    for job_obj in jobs_list:
        job_obj = await sync_job_status(db, job_obj)
...
async def get_device_latest_job(
    *,
    db: AsyncSession = Depends(deps.get_async_db),
    device_id: uuid.UUID,
    current_user: User = Depends(deps.get_current_active_user),
    ...
    # Get latest job
    job_obj = await job.get_latest_by_device(db, device_id=device_id)
    if not job_obj:
        raise HTTPException(
            status_code=404,
...
async def get_job_status(
    *,
    ...
    job_obj = await job.get_by_job_id(db, job_id=job_id)
    if not job_obj:
        raise HTTPException(
            status_code=404,
    ...
    # Check access
    db_device = await device.get_by_id(db, device_id=job_obj.device_id)
    await check_device_access(current_user, db_device, action="view jobs for")
...
async def cancel_job(
    *,
    ...
    # Check access
    db_device = await device.get_by_id(db, device_id=job_obj.device_id)
    await check_device_access(current_user, db_device, action="cancel jobs for")
    ...
    *,
    ...
    async def cleanup_task():
        ...
        async with AsyncSessionLocal() as db_session:
            ...
            all_devices_result = await db_session.execute(select(device.model).filter(device.model.thing_name != None))
            ...
            for dev in all_devices:
                ...
                jobs_to_archive = await job.get_archivable_jobs_for_device(db_session, device_id=dev.device_id, keep_latest=keep_latest)
                ...
                for job_to_archive in jobs_to_archive:
                    ...
                    if success:
                        ...
                        await job.update_status(db_session, job_id=job_to_archive.job_id, status=JobStatus.ARCHIVED)
                        total_cleaned += 1
                    else:
                        total_failed += 1
...
async def get_device_latest_deployment_job(
    *,
    db: AsyncSession = Depends(deps.get_async_db),
    device_id: uuid.UUID,
    current_user: User = Depends(deps.get_current_active_user),
    ...
    job_obj = await job.get_latest_by_device_and_type(db, device_id=device_id, job_type=JobType.PACKAGE_DEPLOYMENT)
