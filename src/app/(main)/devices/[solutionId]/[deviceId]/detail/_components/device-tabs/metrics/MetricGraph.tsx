...
export default function MetricGraph({
  ...
  // データの日数を計算する関数
  const { hours, isHourlyDisplay } = useMemo(() => {
  ...
  // Chart component that can be reused in both main view and modal
  const ChartContent = ({
    height = 256,
    modalView = false,
  }: {
        ...
        <AreaChart
          ...
          <XAxis
            ...
            tick={{
              ...
              fontSize: modalView ? axisFontSize + 2 : axisFontSize,
            }}
            angle={-45}
            textAnchor="end"
            height={60}
          ...
          <Tooltip
            ...
            formatter={(value, name) => {
              // valueがnullまたはundefinedの場合は「データなし」を表示
          ...
          <Legend iconSize={modalView ? 12 : 10} iconType="circle" />
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          {!isLoading && data.length > 0 && (
            <button
              ...
              aria-label="グラフを拡大表示">
              <Maximize2 className="w-4 h-4 text-gray-600 cursor-pointer" />
            </button>
          )}
        </div>
        {isLoading ? (
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-[80vw] max-h-[90vh]">
          ...
          <div className="mt-4 overflow-y-auto max-h-[calc(90vh-120px)]">
            <ChartContent height={500} modalView={true} />
          </div>
        </DialogContent>
      </Dialog>
    </>
