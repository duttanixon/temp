...
def test_get_devices_with_solutions_admin(client: TestClient, admin_token: str):
    """Test admin getting all devices with their solution information"""
    response = client.get(
    ...
    data = response.json()
    assert isinstance(data, list)
    assert len(data) >= 4  # We have at least 4 devices in our test data
    
    # Verify response contains both devices with and without solutions
    devices_with_solutions = [d for d in data if d["current_solution_name"] is not None]
    devices_without_solutions = [d for d in data if d["current_solution_name"] is None]
    
    assert len(devices_with_solutions) > 0
    assert len(devices_without_solutions) > 0
    
    # Verify the structure of a device with solution
    device_with_solution = devices_with_solutions[0]
    assert device_with_solution["current_solution_id"] is not None
    assert device_with_solution["current_solution_name"] == "Test Solution"
    assert device_with_solution["current_solution_status"] == "ACTIVE"
    assert device_with_solution["deployment_id"] is not None
    
    # Verify the structure of a device without solution
    device_without_solution = devices_without_solutions[0]
    assert device_without_solution["current_solution_id"] is None
    assert device_without_solution["current_solution_name"] is None
    assert device_without_solution["current_solution_status"] is None
    assert device_without_solution["deployment_id"] is None


def test_get_devices_with_solutions_filtered_by_customer(client: TestClient, admin_token: str, customer: Customer):
    """Test getting devices with solutions filtered by customer"""
    response = client.get(
        f"{settings.API_V1_STR}/devices/with-solutions?customer_id={customer.customer_id}",
        headers={"Authorization": f"Bearer {admin_token}"}
    )
...
def test_get_devices_with_solutions_with_deployed_solution(
    mock_get_solution, mock_get_device_solution, 
    client: TestClient, admin_token: str, active_device: Device, db: Session
):
    """Test getting devices with an actively deployed solution"""
    # Mock a device solution
    mock_device_solution = MagicMock()
    mock_device_solution.id = uuid.uuid4()
    mock_device_solution.solution_id = uuid.uuid4()
    mock_device_solution.status = "ACTIVE"
    mock_get_device_solution.return_value = [mock_device_solution]
    ...
    # Find our active device in the response
    active_device_data = next(
        (item for item in data if str(item["device_id"]) == str(active_device.device_id)), 
        None
    )
    assert active_device_data is not None
    
    # Verify solution info was added
    assert active_device_data["current_solution_id"] is not None
    assert active_device_data["current_solution_name"] == "Test Solution"
    assert active_device_data["current_solution_status"] == "ACTIVE"
    assert active_device_data["deployment_id"] is not None

@patch('app.crud.device_solution.device_solution.get_by_device')
def test_get_devices_with_solutions_without_deployed_solution(
    mock_get_device_solution, client: TestClient, admin_token: str
):
    """Test getting devices without any deployed solutions"""
    # Mock empty device solution list
    mock_get_device_solution.return_value = []
    ...
    # Verify solution fields are null for devices without solutions
    for device_item in data:
        assert device_item["current_solution_id"] is None
