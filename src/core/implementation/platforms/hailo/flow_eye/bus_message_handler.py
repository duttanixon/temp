...
class BusMessageHandler:
    ...
    def __init__(self, pipeline_manager: 'PipelineManager', loop: 'GLib.MainLoop', gst_context: 'GstContext', isDebug: bool = False):
    ...
    def _init_log_file(self):
        ...
        try:
            ...
            self.log_to_file(f"HAILO PIPELINE SESSION STARTED AT {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')}")
            self.log_to_file(f"{'='*80}")
            ...
            if pipeline:
                ...
                for element in elements:
                    try:
                        ...
                        # get element pads
                        for pad_type, pad_iterator in [("Src Pads", element.iterate_src_pads()),
                            ...
                            if pads:
                                ...
                                for pad in pads:
                                    ...
                                    caps_str = "None" if caps is None else caps.to_string()
        ...
        if hailo_elements:
            ...
            for element in hailo_elements:
                ...
                if 'HailoNet' in element_type:
                    try:
                        hef_path = element.get_property('hef-path') if hasattr(element.props, 'hef-path') else "Unknown"
                        batch_size = element.get_property('batch-size') if hasattr(element.props, 'batch-size') else "Unknown"
                    ...
                    try:
                        so_path = element.get_property('so-path') if hasattr(element.props, 'so-path') else "Unknown"
                        function_name = element.get_property('function-name') if hasattr(element.props, 'function-name') else "Unknown"
                    ...
                    try:
                        module = element.get_property('module') if hasattr(element.props, 'module') else "Unknown"
        ...
        if (current_time - self.last_stats_time) >= self.stats_interval_sec:
            ...
            if self.qos_stats:
                ...
                for element, stats in self.qos_stats.items():
                    ...
                    drop_rate = (dropped / (processed + dropped)) * 100 if (processed + dropped) > 0 else 0
                    self.log_to_file(f" {element}: Dropped  {dropped} of {processed + dropped} buffers ({drop_rate:.2f}%)")
    ...
    def __del__(self):
        ...
        if self.isDebug and self.log_file:
            try:
                ...
                self.log_to_file(f"GStreamer session ended at {time.strftime('%Y-%m-%d %H:%M:%S')}")
                self.log_to_file(f"{'#'*40}")
                self.log_file.close()
            except:
                pass
    ...
    def bus_call(self, bus, message, *args) -> bool:
        ...
        if msg_type == self.context.gst.MessageType.EOS:
            ...
            if self.isDebug:
                ...
                try:
                    ...
                    if state_result == self.context.gst.StateChangeReturn.SUCCESS:
                        ...
                        self.log_to_file(f"  Element state: {state_name}, pending: {pending_name}")
                    else:
            ...
            if self.isDebug:
                ...
                if src_element == self.pipeline_manager.get_pipeline():
                    self.log_to_file(f"PIPELINE STATE CHANGED: {old_state_name} → {new_state_name} (pending: {pending_state_name})")
                    ...
                    self.log_to_file(f"HAILO ELEMENT '{src_name}' STATE CHANGED: {old_state_name} → {new_state_name} (pending: {pending_state_name})")
                else:
                    self.log_to_file(f"Element '{src_name}' state: {old_state_name} → {new_state_name}")      
        ...
        # All remaining handlers follow the same pattern - only process and log if isDebug is True
            ...
            if structure:
                ...
                qos_data = {
                    'live': structure.get_value("live") if structure.has_field("live") else None,
                    'running_time': structure.get_value("running-time") if structure.has_field("running-time") else None,
                    'stream_time': structure.get_value("stream-time") if structure.has_field("stream-time") else None,
                    'timestamp': structure.get_value("timestamp") if structure.has_field("timestamp") else None,
                    'duration': structure.get_value("duration") if structure.has_field("duration") else None,
                    'dropped': structure.get_value("dropped") if structure.has_field("dropped") else None,
                    'processed': structure.get_value("processed") if structure.has_field("processed") else None,
                    'jitter': structure.get_value("jitter") if structure.has_field("jitter") else None,
                    'quality': structure.get_value("quality") if structure.has_field("quality") else None,
                }
                ...
                if qos_data['dropped'] is not None and qos_data['processed'] is not None:
                    total = (qos_data['dropped'] or 0) + (qos_data['processed'] or 0)
                    drop_rate = (qos_data['dropped']/total) * 100 if total > 0 else 0
                    self.log_to_file(f"    Dropped: {qos_data['dropped']} of {total} buffers ({drop_rate:.2f}%)")
                ...
                self.log_to_file(f"QoS message from {src_name} (no structure available)")
        ...
        # you'd include all the detailed handling from your original code)
            ...
            if structure:
                ...
                self.log_to_file(f"Element message from {src_name}, type: {structure_name}")
                    ...
                    try:
                        ...
                        for i in range(n_fields):
                            ...
                            try:
                                ...
                                if 'inference' in field_name.lower() or 'latency' in field_name.lower():
                                    if src_name not in self.hailo_inference_stats:
                                        self.hailo_inference_stats[src_name] = {}
                                    self.hailo_inference_stats[src_name][field_name] = field_value
                            except Exception as e:
                                self.log_to_file(f"     {field_name}: <cannot display: {e}>")
                    except Exception as e:
                ...
                if structure_name == "GstBinForwarded":
                    ...
                    if forwarded_msg:
                        ...
                        fwd_src = forwarded_msg.src.get_name() if forwarded_msg.src else "unknown"
                        self.log_to_file(f"     Forwarded message from {fwd_src} of type {fwd_type}")

                elif "level" in structure_name:
                    # Audio/video level message
                    if structure.has_field("rms"):
                        rms_values = structure.get_value("rms")
                        peak_values = structure.get_value("peak") if structure.has_field("peak") else None
                        self.log_to_file(f"     Level information - RMS: {rms_values}, Peak: {peak_values}")
                
                elif "stream-start" in structure_name:
                    # start stream message
                    uri = structure.get_value("uri") if structure.has_field("uri") else None
                    self.log_to_file(f"    Stream started: {uri}")
                
                elif "progress" in structure_name:
                    # Progress message
                    percent = structure.get_value("percent-double") if structure.has_field("percent-double") else structure.get_value("percent")
                    code = structure.get_value("code") if structure.has_field("code") else None
                    text = structure.get_value("text") if structure.has_field("text") else None
                    self.log_to_file(f"  Progress: {percent}% - {code}: {text}")                
            else:
                self.log_to_file(f"Element message from {src_name} (no structure available)")
        ...
        # Add similar conditional checks for the remaining message types
            ...
            percent = message.parse_buffering_stats()[0] if hasattr(message, 'parse_buffering_stats') else message.parse_buffering()
        ...
        # For all other message types, only log details if debug is enabled
            ...
            if structure:
                ...
                for i in range(n_fields):
                    try:
                        ...
                        self.log_to_file(f"    {field_name} ({field_type}): {field_value}")
                    except Exception as e:
            ...
            try:
                ...
                for attr in ['get_stream_status_object', 'parse_group_id', 'parse_context_type']:
                    if hasattr(message, attr):
                        try:
                            value = getattr(message, attr)()
                            self.log_to_file(f"  {attr.replace('get_', '').replace('parse_', '')}: {value}")
                        except:
                            pass
