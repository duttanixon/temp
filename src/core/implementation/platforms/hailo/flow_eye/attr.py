...
# Helper function to determine the top attribute index
def top1(array: np.ndarray):
    return np.argsort(array[0:3])[
        0
    ]  # Return the index of the max value among the first 3 elements
...
def run(video_frame: VideoFrame):
    try:
        if not video_frame.roi:
            print("Warning: No ROI in video frame")
        ...
        try:
            attributes = (
                np.squeeze(np.array(results, copy=False)).astype(np.float32) / 255.0
            )
            gender = classify_gender(attributes[16])
            age, age_confidence = classify_age(attributes)

            video_frame.roi.add_object(
                hailo.HailoClassification("gender", 0, gender, attributes[16])
            )
            video_frame.roi.add_object(
