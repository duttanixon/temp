...
interface ShadcnPieChartLabelCardProps {
  title: string;
  fontSize: number;
  description?: string;
...
export default function ShadcnPieChartLabelCard({
  title,
  description,
  data,
  isLoading,
  ...
  const renderPieChart = (isFullView: boolean) => {
    ...
    return (
      <>
        <ChartContainer
          config={chartConfig}
          className="max-h-[300px] pb-0 [&_.recharts-pie-label-text]:fill-foreground mx-auto aspect-square"
        >
          <RechartsPieChart>
            <ChartTooltip
              cursor={false}
            ...
            <Pie
              ...
              startAngle={90}
              endAngle={-270}
              labelLine={false} // Set true if ２ion="outside" for labels
              label={({ payload, percent, x, y, innerRadius, midAngle }) => {
                const RADIAN = Math.PI / 180;
                const radius = innerRadius * 0.2;
                const cx = x ?? 0;
                const cy = y ?? 0;
