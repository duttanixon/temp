...
import { AddAdminAccessContent } from "@/app/(main)/users/add/_components/AddAdminAccessContent";
import { AddUserButton } from "@/app/(main)/users/add/_components/AddUserButton";
type Props = {
  accessToken: string;
};
export default function AddAdminUserForm({ accessToken }: Props) {
  const form = useUserFormCreate(accessToken, "ADMIN");
      ...
      <div className="flex flex-col gap-8">
        ...
        <form onSubmit={form.handleSubmit} className="flex flex-col gap-4">
          <div className="w-170 h-145 border border-[#BDC3C7] rounded bg-[#FFFFFF]">
            ...
            <AddAdminAccessContent {...form} accessToken={accessToken} />
          </div>
          <AddUserButton {...form} />
        </form>
      </div>
