...
export default function DeployToDeviceModal({ 
    ...
    useEffect(() => {
        ...
        try {
            ...
            const availableCustomers = await deviceSolutionService.getCustomersWithSolutionAccess(solution.solution_id);
    ...
    const onSubmit = async (data: DeployToDeviceFormValues) => {
        ...
        try {
          ...
          const deployment = await deviceSolutionService.deployToDevice(deploymentData);
        ...
        <Dialog open={true} onOpenChange={onClose}>
          <DialogContent className="sm:max-w-[500px]">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="device_id" className="text-sm font-medium text-[#7F8C8D]">
                ...
                {errors.device_id && (
                  <p className="text-xs text-red-500 mt-1">{errors.device_id.message}</p>
                )}
              ...
              <div className="space-y-2">
                <Label htmlFor="version_deployed" className="text-sm font-medium text-[#7F8C8D]">
                ...
                {errors.version_deployed && (
                  <p className="text-xs text-red-500 mt-1">{errors.version_deployed.message}</p>
                )}
              </div>
              ...
              {solution.configuration_template && (
                <div className="space-y-2">
                  <Label htmlFor="configuration" className="text-sm font-medium text-[#7F8C8D]">
                    構成設定
                  </Label>
                  <Controller
                    name="configuration"
              ...
              <DialogFooter className="pt-4">
                ...
                <Button 
                  type="submit"
                  disabled={isLoading || !!(selectedCustomerId && devices.length === 0)}
